...
export async function getTournamentLeaderboard(tournamentId: string) {
    ...
    if (userIds.length > 0) {
        // Find users with ANY unresolved prediction that started in the past (Live)
        // 1. Single Votes
        const { data: liveVotes } = await supabase
            .from('votes')
            .select('user_id, prediction:predictions!inner(expires_at, resolved)')
            .in('user_id', userIds)
            .eq('prediction.resolved', false)
            .lt('prediction.expires_at', now);
        ...
        return data.map((entry: any) => ({
            ...entry,
            has_live_bets: liveUserSet.has(entry.user_id)
        }));
    }
...
export async function joinTournamentWithBalance(tournamentId: string) {
    ...
    const { error: joinError } = await adminClient.from("tournament_entries").insert({
        ...
        user_id: user.id,
        current_stack: tournament.starting_stack || 1000,
        paid: true, // Restored: SQL script will ensure this column exists
